...
setup: ## [lite] Create venv + install deps (~80 MB, ~10s with pip / ~2s with uv)
	@if [ ! -x "$(PY)" ]; then \
	  if command -v uv >/dev/null 2>&1; then uv venv $(VENV) --python '>=3.10,<3.14'; \
	...
	@$(JUPYTEXT) --to notebook --update notebooks/0*.py 2>/dev/null || $(JUPYTEXT) --to notebook notebooks/0*.py
	@echo ""
...
lab: ## [lite] Open Jupyter Lab on http://localhost:8888
	@$(JUPYTEXT) --to notebook --update notebooks/0*.py 2>/dev/null || true
